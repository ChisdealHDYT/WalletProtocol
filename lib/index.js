...
if (npm_package) {
    console.log("--- ZENZO Protocol (SCP) Wallet v" + npm_package.version + " --- ");    
} else {
...
let appdataCore = process.env.APPDATA || (process.platform == 'darwin' ? process.env.HOME + '/Library/Application Support/Zenzo/' : process.env.HOME + '/.zenzo/');
if (appdataCore === process.env.APPDATA) appdataCore += '/Zenzo/'; // Append '/StakeCubeCoin/' to the windows appdata directory
...
async function init() {
    ...
    try {
        ...
        let conf = await DB.fromDiskCore("zenzo.conf");
        // Make sure there's atleast *something* in the config
        if (!conf || conf.length === 0) {
            console.warn("No ZENZO Core config file detected!");
            return false;
        }
